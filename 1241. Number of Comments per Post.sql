...
with cte as (
Select distinct  S1.sub_id as post_id  from 
submissions S1 
where S1.parent_id  is null 
)

select  C.post_id , case when 
parent_id is not null then count(S.parent_id) else 0 end as number_of_comments 
from cte  C
left join (Select distinct sub_id ,parent_id  from submissions where parent_id is not null) S
on C.post_id=S.parent_id
group by post_id,S.parent_id
